...
static int scull_p_open(struct inode *inode, struct file *filp)
{
	...
	if (filp->f_mode & FMODE_WRITE) { //producer
		// dev->wp += (dev->space_remaining!=scull_p_num_buffer) ? ( (scull_p_num_buffer - dev->space_remaining)*SIZE_OF_ITEM ) : (0); /* dont overwrite */
		++(dev->nwriters);
	}
...
//	if ( (dev->space_remaining > 0) && ( ((size_t)(dev->end - dev->wp)) >= SIZE_OF_ITEM) )
...
//	else {
//		up (&dev->sem);
//		PDEBUG("Not enough space in skullbuffer\n");
//		return 0;
//	}
	dev->wp += SIZE_OF_ITEM;
	if (dev->wp == dev->end)
